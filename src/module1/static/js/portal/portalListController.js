(function() {
  ...
  PortalListController.$inject = ['$scope', '$filter', '$window', 'Agent', 'PortalFactory', 'AgentService', 'LabelService', '$log', '$mdBottomSheet', '$q', '$timeout', '$location', '$stateParams']
  function PortalListController($scope, $filter, $window, Agent, Portal, AgentService, LabelService, $log, $mdBottomSheet, $q, $timeout, $location, $stateParams) {
    $scope.newlabel = {};
    $scope.map = {
      center: {
        latitude: 45,
  ...
  angular.module('goGress').filter('agentCodeNameFromId', agentCodeNameFromId);
